...
sudo apt install build-essential checkinstall libncursesw5-dev libssl-dev libsqlite3-dev tk-dev libgdbm-dev libc6-dev libbz2-dev libffi-dev libgsl-dev libmath-random-mt-perl wget unzip
# Install Python 2.7.18
wget https://www.python.org/ftp/python/2.7.18/Python-2.7.18.tgz
tar -xzf Python-2.7.18.tgz
cd Python-2.7.18
./configure --enable-optimizations --prefix=$CURDIR/Python-2.7.18
make -j$NCPU
make install
# Install ecoPCR
...
mkdir -p taxonomy
cd taxonomy
wget -c https://ftp.ncbi.nih.gov/pub/taxonomy/taxdump_archive/taxdmp_2023-01-01.zip
unzip taxdmp_2023-01-01.zip -d taxonomy
rm taxdmp_2023-01-01.zip
cd ..
...
Python-2.7.18/bin/python2.7 ./ecoPCRFormat.py -f -t ./taxonomy -n 12Sreferences 12Sreferences.ecoPCR
...
vsearch --fasta_width 0 --notrunclabels --threads $NCPU --cluster_fast 12Sbarcodes.fasta --qmask none --id 0.9 --strand plus --centroids 12Sbarcodes_representatives.fasta
...
ls Sample??.fasta | grep -o -P '^[^\.]+' | xargs -P $NCPU -I {} sh -c './Simera-master/Simera -i {}.fasta -o {}_simeraout -n 50 -s 10000 -l 0.0001 -c 10000 -f GTCGGTAAAACTCGTGCCAGC -r CATAGTGGGGTATCTAATCCCAGTTTG'
ls Blank??.fasta | grep -o -P '^[^\.]+' | xargs -P $NCPU -I {} sh -c './Simera-master/Simera -i {}.fasta -o {}_simeraout -n 50 -s 100 -l 0.0001 -c 10000 -f GTCGGTAAAACTCGTGCCAGC -r CATAGTGGGGTATCTAATCCCAGTTTG'
# Replace abundance format
ls Sample??.fasta Blank??.fasta | grep -o -P '^[^\.]+' | xargs -P $NCPU -I {} sh -c 'perl -i -npe "if(/^>/){s/_(\\d+)\\n/\\;size=\$1\\n/;}" {}_simeraout/samp_all_seqs.fa'
# Rereplicate sequences
ls Sample??.fasta Blank??.fasta | grep -o -P '^[^\.]+' | xargs -P $NCPU -I {} sh -c 'vsearch --fasta_width 0 --notrunclabels --rereplicate {}_simeraout/samp_all_seqs.fa --sizein --xsize --output {}_simeraout/samp_rerep.fasta'
# Generate simulated sequences
ls Sample??.fasta Blank??.fasta | grep -o -P '^[^\.]+' | xargs -P $NCPU -I {} sh -c './art_bin_MountRainier/art_illumina --amplicon --seqSys MSv1 --in {}_simeraout/samp_rerep.fasta --len 144 --paired --noALN --fcov 1 --out {}_'
# Make demultiplexed FASTQ
ls Sample??.fasta Blank??.fasta | grep -o -P '^[^\.]+' | xargs -P $NCPU -I {} sh -c 'perl addNNNNNN.pl {}_1.fq > {}_R1_001.fastq; perl addNNNNNN.pl {}_2.fq > {}_R2_001.fastq'
# Make output folder
...
ls *.fastq | xargs -L 1 -P $NCPU gzip -9
mv *.fastq.gz ../../01a_RawSequences_woSTD/
cd ..
rm -rf TEMP

# Make "wSTD" sequences
ls Sample??.fasta | grep -o -P '^[^\.]+' | xargs -P $NCPU -I {} sh -c 'cat {}.fasta MiFish_STD_*.fasta > {}_wSTD.fasta'
ls Blank??.fasta | grep -o -P '^[^\.]+' | xargs -P $NCPU -I {} sh -c 'cat {}.fasta > {}_wSTD.fasta; cat MiFish_STD_*.fasta | perl -npe "if(/^>/){s/\$/0/}" >> {}_wSTD.fasta'
# Perform chimera formation simulation
ls Sample??_wSTD.fasta | grep -o -P '^[^\.]+' | xargs -P $NCPU -I {} sh -c './Simera-master/Simera -i {}.fasta -o {}_simeraout -n 50 -s 20000 -l 0.0001 -c 10000 -f GTCGGTAAAACTCGTGCCAGC -r CATAGTGGGGTATCTAATCCCAGTTTG'
ls Blank??_wSTD.fasta | grep -o -P '^[^\.]+' | xargs -P $NCPU -I {} sh -c './Simera-master/Simera -i {}.fasta -o {}_simeraout -n 50 -s 10000 -l 0.0001 -c 10000 -f GTCGGTAAAACTCGTGCCAGC -r CATAGTGGGGTATCTAATCCCAGTTTG'
# Replace abundance format
ls Sample??_wSTD.fasta Blank??_wSTD.fasta | grep -o -P '^[^\.]+' | xargs -P $NCPU -I {} sh -c 'perl -i -npe "if(/^>/){s/_(\\d+)\\n/\\;size=\$1\\n/;}" {}_simeraout/samp_all_seqs.fa'
# Rereplicate sequences
ls Sample??_wSTD.fasta Blank??_wSTD.fasta | grep -o -P '^[^\.]+' | xargs -P $NCPU -I {} sh -c 'vsearch --fasta_width 0 --notrunclabels --rereplicate {}_simeraout/samp_all_seqs.fa --sizein --xsize --output {}_simeraout/samp_rerep.fasta'
# Generate simulated sequences
ls Sample??_wSTD.fasta Blank??_wSTD.fasta | grep -o -P '^[^\.]+' | xargs -P $NCPU -I {} sh -c './art_bin_MountRainier/art_illumina --amplicon --seqSys MSv1 --in {}_simeraout/samp_rerep.fasta --len 144 --paired --noALN --fcov 1 --out {}_'
# Make demultiplexed FASTQ
ls Sample??_wSTD.fasta Blank??_wSTD.fasta | grep -o -P '^[^\.]+' | xargs -P $NCPU -I {} sh -c 'perl addNNNNNN.pl {}_1.fq > {}_R1_001.fastq; perl addNNNNNN.pl {}_2.fq > {}_R2_001.fastq'
# Make output folder
